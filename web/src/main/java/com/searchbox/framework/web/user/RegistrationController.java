...
public class RegistrationController {
  ...
  protected static final String VIEW_NAME_REGISTRATION_PAGE = "user/register";
  ...
  public String registerUserAccount(
      ...
      BindingResult result, WebRequest request) throws DataException {
    ...
    UserEntity registered = createUserAccount(userAccountData, result);

    // If email address was already found from the database, render the form
    // view.
    if (registered == null) {
      LOGGER
          .info("An email address was found from the database. Rendering form view.");
      return VIEW_NAME_REGISTRATION_PAGE;
    }
    ...
    // If the user is signing in by using a social provider, this method call
    // stores
    // the connection to the UserConnection table. Otherwise, this method does
    // not
    // do anything.
   ...
   */
  private UserEntity createUserAccount(RegistrationForm userAccountData,
  ...
  private void addFieldError(String objectName, String fieldName,
    ...
    FieldError error = new FieldError(objectName, fieldName, fieldValue, false,
