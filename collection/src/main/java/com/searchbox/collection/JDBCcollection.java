...
public abstract class JDBCcollection extends AbstractBatchCollection implements
    InitializingBean {
  ...
  protected String getSelectClause() {
    return query.substring(query.indexOf("SELECT") + 6, query.indexOf("FROM"));
  }
  ...
  protected String getWhereClause() {
    return query.substring(
        Math.max(query.indexOf("WHERE") + 5, query.length()),
    ...
    return new RowMapper<FieldMap>() {
      ...
      public FieldMap mapRow(ResultSet result, int rowNum) throws SQLException {
        ...
        for (int i = 0; i < meta.getColumnCount(); i++) {
          ...
          LOGGER.debug("Adding to map -> [{}]: {}", meta.getColumnLabel(i + 1),
  ...
  protected FlowJobBuilder getJobFlow(JobBuilder builder) {
    ...
    try {
      ...
      indexItems = stepBuilderFactory.get("indexItems")
          .<FieldMap, FieldMap> chunk(200).reader(jdbcReader())
          .processor(processor()).writer(fieldMapWriter())
