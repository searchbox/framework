...
public class TopicCollection extends AbstractBatchCollection implements
	...
	private String getCallDescription(String callFileName) {
		...
		if (!callList.get(callFileName).containsKey("descriptionHtml")) {
			...
			callList.get(callFileName).put("descriptionHtml", htmlValue);
		}

		return callList.get(callFileName).get("descriptionHtml").toString();
	}
	...
	public ItemProcessor<JSONObject, FieldMap> topicProcessor() {
		return new ItemProcessor<JSONObject, FieldMap>() {
			...
			public FieldMap process(JSONObject topicObject) throws IOException {
				 ...
				 *  {
					"tags":[],
					"callTitle":"Marie Skaodowska-Curie Research and Innovation Staff Exchange (RISE) ",
					"title":"Marie Skaodowska-Curie Research and Innovation Staff Exchange (RISE)",
					"flags":["Gender","IntlCoop","SSH"],
					"callIdentifier":"H2020-MSCA-RISE-2014",
					"description":"Objective:\r\n\r\nThe RISE scheme will promote international and inter-sector collaboration \r\nthrough",
					"callFileName":"h2020-msca-rise-2014",
					"callDeadline":1398351600000,
					"callStatus":"Open",
				...
				doc.put("callTitle", (String) topicObject.get("callTitle"));
				...
				doc.put("callStatus", (String) topicObject.get("callStatus"));
	...
	public ItemProcessor<JSONObject, FieldMap> callProcessor() {
		return new ItemProcessor<JSONObject, FieldMap>() {
			...
			public FieldMap process(JSONObject callObject) throws IOException {
				 ...
				 *  {
					"CallIdentifier":{
						"FileName":"h2020-msca-itn-2014",
						"CallId":"H2020-MSCA-ITN-2014",
						"Status":"OPEN"
					},
					"Title":"MARIE SKAODOWSKA-CURIE ACTION: INNOVATIVE TRAINING NETWORKS (ITN)",
					...
					"SP_color":"EU_1",		
					"PublicationDate":1386716400000,
					"DeadlineDate":1397055600000,
					"CallDetails":{
						"AdditionalInfo":"",
						"LatestInfo":{
							"ApprovalDate":1391177582681,
							"Content":"The submission session is now available for: MSCA-ITN-2014-EID(MSCA-ITN-EID), MSCA-ITN-2014-EJD(MSCA-ITN-EJD),MSCA-ITN-2014-ETN(MSCA-ITN-ETN)"
							}
						},
				...
				JSONObject callIdentifierObject = (JSONObject) callObject
						.get("CallIdentifier");
				String callIdentifier = (String) callIdentifierObject
						.get("CallId");
				String callFileName = (String) callIdentifierObject
						.get("FileName");
				...
				enhancedCall.put("eur_call_timestamp", Calendar.getInstance()
						.getTime());
				...
				// Saving the enhanced file
				try {
					File file = new File("output/calls/" + callFileName
							+ ".json");
					file.getParentFile().mkdirs();
				...
				doc.put("id", callIdentifier);
				doc.put("title", (String) callObject.get("Title"));
				doc.put("descriptionRaw", callDetailRaw);
				doc.put("descriptionHtml", callDetailHtml);
				doc.put("docType", "Call H2020");
				doc.put("programme", "H2020");

				doc.put("programmeDescription", (String) callObject
						.get("MainSpecificProgrammeLevel1Description"));
				doc.put("callPublication",
						(Long) callObject.get("PublicationDate"));
				doc.put("callDeadline", (Long) callObject.get("DeadlineDate"));
				doc.put("frameworkProgramme",
				...
				// ("TotalIndicativeBudget":"405,180,000")
				try {
					Long num = (Long) NumberFormat.getNumberInstance(
							java.util.Locale.US).parse(
							(String) callObject.get("TotalIndicativeBudget"));
				...
				doc.put("callStatus",
						(String) callIdentifierObject.get("Status"));
		...
		return new ItemWriter<FieldMap>() {
			...
			public void write(List<? extends FieldMap> items) throws Exception {
				...
				for (FieldMap item : items) {
					...
					LOGGER.info("Adding call {} to the list of calls", 
							item.get("id").toString());
					
					callList.put(item.get("id").toString(), item);
				}
