...
public class EdismaxQuerySolrAdaptor {
    ...
    public void setQueryFields(SolrSearchEngine searchEngine,
            EdismaxQuery SearchElement, SolrQuery query,
        ...
        if (fieldAttribute.getSearchable()) {
            LOGGER.debug("Found searchable field: {}", fieldAttribute
                    .getField().getKey());
            Float boost = (fieldAttribute.getBoost() != null) ? fieldAttribute
                    .getBoost() : 1.0f;
            String key = searchEngine
                    .getKeyForField(fieldAttribute, USE.SEARCH);
            ...
            query.set(
                    ...
                    ((currentFields != null && !currentFields.isEmpty()) ? currentFields
                            + " "
                            : "")
                            + ((key == null) ? fieldAttribute.getField()
                                    .getKey() : key) + "^" + boost);
        }
    }
    ...
    public void getSugestions(SolrSearchEngine engine,
        ...
        for (String suggester : data.keySet()) {
            NamedList<Object> suggestions = (NamedList<Object>) data.get(
                    suggester).get(condition.getQuery());
